...
def _run_oie_migration(conn):
    """Run OIE (Opportunity Intelligence Engine) migration for v17.18 support."""
    ...
    if os.path.exists(OIE_MIGRATION_PATH):
        logger.info("Running OIE v17.18 migration...")
        with open(OIE_MIGRATION_PATH, 'r') as f:
            migration_sql = f.read()
        conn.executescript(migration_sql)
        conn.commit()
        logger.info("OIE v17.18 migration complete")
    else:
...
def init_db(db_path=None):
    ...
    try:
        ...
        # Run OIE migration for v17.18 opportunity tracking
