...
v17.20 (current) and legacy v17.12.3 payload formats.
...
def normalize_oie_payload(payload: dict) -> dict:
    ...
    Handles:
    - v17.20 Sniper alerts (entry_price, stop_loss, take_profit)
    - v17.20 Retrace alerts (suggested_entry, target_sl, target_tp)
    ...
    # Support both v17.20 field names and v17.14 shorthand with fallbacks
...
def oie_to_legacy_compact(payload: dict) -> dict:
    """
    Convert an OIE v17.20 payload into the compact format that the existing
    ...
    return {
        ...
        "rr": 3.0,
        "t": ts,
        "v": payload.get("version", "v17.20").replace("v", ""),
