...
v17.18 (current) and legacy v17.12.3 payload formats.
...
def normalize_oie_payload(payload: dict) -> dict:
    ...
    Handles:
    - v17.18 Sniper alerts (entry_price, stop_loss, take_profit)
    - v17.18 Retrace alerts (suggested_entry, target_sl, target_tp)
    ...
    # Support both v17.18 field names and v17.14 shorthand with fallbacks
...
def oie_to_legacy_compact(payload: dict) -> dict:
    """
    Convert an OIE v17.18 payload into the compact format that the existing
    ...
    return {
        ...
        "rr": 3.0,
        "t": ts,
        "v": payload.get("version", "v17.18").replace("v", ""),
